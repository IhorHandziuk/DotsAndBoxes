...
void display() {
    ...
    if (f_p_score + s_p_score == box_num) {
        if (f_p_score > s_p_score) {
            sprintf(winner, "first player won");
            norm = 110;
        }
        else if (f_p_score < s_p_score) {
            sprintf(winner, "second player won");
            norm = -40;
        }
        else {
            sprintf(winner, "draw");
                    ...
                    if ((sides(boxes[i]) == 2) && (sides(*boxes[i].neig[way]) == 2)) {
                        for (GLint w = 0; w < 4; w++) {
                            ...
                            if (!(*boxes[i].neig[way]).dir[w].painted) {
                                if ((!(*boxes[i].neig[way]).neig[w]) || (sides(*(*boxes[i].neig[way]).neig[w]) == 1))
                                    two++;
                            }
                        }
                        if (two == 2) {
...
void mouse_action(int button, int state, int tx, int ty) {
    if (button == GLUT_LEFT_BUTTON && state == GLUT_DOWN) {
        ...
        for (GLint i = 0; i < box_num; i++) {
            for (GLint way = 0; way < 4; way++)
                if (dist(boxes[i].dir[way].x1 * k, boxes[i].dir[way].y1, boxes[i].dir[way].x2 * k,
                         boxes[i].dir[way].y2) + 0.25 >
                    dist(x, y, boxes[i].dir[way].x1 * k, boxes[i].dir[way].y1) +
                    dist(x, y, boxes[i].dir[way].x2 * k, boxes[i].dir[way].y2)) {
